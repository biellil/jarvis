...
must_haves:
  truths:
    - "Script polls GET /internal/diagnostics every 30 minutes (configurable via --duration)"
    - "Script calculates deltas against baseline and applies QA-01 thresholds: heap <100MB, RSS <200MB, p99 <50ms, audioContextCount must equal 1"
    - "Script generates a soak-report-{timestamp}.html with Chart.js line charts showing heap, RSS, event loop p99 over time with FAIL threshold marker lines"
    - "Script accepts --duration flag for quick tests (e.g., --duration 60000 runs for 1 minute)"
    - "Script exits with code 0 on PASS, code 1 on FAIL"
  artifacts:
    - path: apps/desktop/scripts/soak-test.ts
      provides: "Refactored soak test script with HTTP polling, QA-01 thresholds, HTML report"
      contains: "HEAP_DELTA_FAIL_THRESHOLD_MB = 100"
  key_links:
    - from: apps/desktop/scripts/soak-test.ts
      to: "http://localhost:3000/internal/diagnostics"
      via: "fetch() with AbortSignal.timeout(10_000) every SAMPLE_INTERVAL_MS"
    - from: apps/desktop/scripts/soak-test.ts
      to: "soak-report-{timestamp}.html"
      via: "fs.writeFileSync() at test completion (timeout or SIGINT)"
---

<objective>
Refactor `apps/desktop/scripts/soak-test.ts` from the Phase 44 standalone local script to a QA-01-compliant HTTP polling soak test with:
- HTTP polling of `GET /internal/diagnostics` (from Plan 01) every 30 minutes
- Updated thresholds: heap delta FAIL=100MB, RSS delta FAIL=200MB, event loop p99 FAIL=50ms, audioContextCount FAIL >1
- HTML report with Chart.js CDN visualization of all 4 metrics with FAIL threshold lines
- `--duration` flag for quick smoke tests before running the full 8h

Purpose: This is the user-facing artifact that validates QA-01. The script runs externally while JARVIS is live in Always-Listening mode.
Output: Refactored `soak-test.ts` that produces `soak-report-{timestamp}.html`.
</objective>
...
<interfaces>
<!-- From Phase 44 script (to be replaced) — key constants to update -->
Phase 44 constants (REPLACE ALL):
```typescript
// OLD (Phase 44) → NEW (Phase 56 QA-01)
const HEAP_DELTA_FAIL_THRESHOLD_MB = 10;      // → 100
const RSS_DELTA_WARN_THRESHOLD_MB = 50;        // → remove; replace with RSS_DELTA_FAIL_THRESHOLD_MB = 200
// NEW additions:
const EVENT_LOOP_P99_FAIL_THRESHOLD_MS = 50;
const AUDIO_CONTEXT_FAIL_THRESHOLD = 1; // count > 1 = FAIL
```

DiagnosticsResponse shape from Plan 01 endpoint:
```typescript
interface DiagnosticsResponse {
  heapUsed: number;        // bytes
  rss: number;             // bytes
  eventLoopP99Ms: number;  // milliseconds
  audioContextCount: number;
  timestamp: string;       // ISO string
}
```

Sample structure (replaces Phase 44 MemSample):
```typescript
interface SoakSample {
  time: number;            // Date.now()
  heapUsed: number;        // bytes
  rss: number;             // bytes
  eventLoopP99Ms: number;  // ms
  audioContextCount: number;
}
```

HTML Report requirements (D-03):
- Chart.js via CDN: https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.min.js
- 3 line charts: heap (MB), RSS (MB), event loop p99 (ms)
- Horizontal dashed threshold lines: heap baseline+100MB, RSS baseline+200MB, 50ms
- audioContextCount shown as a table column in samples table
- PASS/FAIL summary section at top

Execution environment (per VALIDATION.md):
- `node --expose-gc apps/desktop/scripts/soak-test.ts` (full 8h)
- `node --expose-gc apps/desktop/scripts/soak-test.ts --duration 60000` (1-min quick)
- GATEWAY_URL env var (default: http://localhost:3000)
</interfaces>
</context>
...
<task type="auto">
  <name>Task 1: Refactor soak-test.ts — HTTP polling + QA-01 thresholds + HTML report</name>
  <files>
    apps/desktop/scripts/soak-test.ts
  </files>
  <read_first>
    - apps/desktop/scripts/soak-test.ts (full Phase 44 script — preserve structure: SIGINT handler, cleanup(), baseline + interval loop)
    - .planning/phases/56-always-listening-soak-test/56-CONTEXT.md (D-03: HTML report spec, D-04: polling, D-07: thresholds)
    - .planning/phases/56-always-listening-soak-test/56-RESEARCH.md (Pattern 1: HTTP polling code; Pattern 4: HTML report template; Pitfall 5: HTTP timeout/retry)
  </read_first>
  <action>
Complete rewrite of `apps/desktop/scripts/soak-test.ts`. Preserve the overall skeleton (constants, baseline, intervalId, soakTimeoutId, cleanup, SIGINT handler) but replace local process.memoryUsage() with HTTP polling.
...
let testPassed = true; // guilty until proven innocent on each sample
...
function generateHtmlReport(baselineSample: SoakSample, allSamples: SoakSample[]): string {
  ...
  const labels = JSON.stringify([
    'baseline',
    ...allSamples.map((s, i) => `t+${(((s.time - baselineSample.time) / 1000 / 60)).toFixed(0)}m`),
  ]);
...
Key changes from Phase 44:
- `MemSample` → `SoakSample` with `eventLoopP99Ms` + `audioContextCount` fields
- `sample()` removed; replaced by `fetchDiagnostics()` HTTP polling
- All 4 QA-01 thresholds enforced
- `generateHtmlReport()` added — produces Chart.js HTML
- `--duration` CLI flag for quick tests
- `GATEWAY_URL` env var support
- Retry logic: 3 attempts with 2s/4s/6s backoff on HTTP failure
  </action>
  <verify>
    <automated>cd apps/desktop && node --expose-gc scripts/soak-test.ts --help 2>&1 | head -5; echo "exit:$?"</automated>
  </verify>
  <acceptance_criteria>
    - apps/desktop/scripts/soak-test.ts contains `HEAP_DELTA_FAIL_THRESHOLD_MB = 100` (not 10)
    - apps/desktop/scripts/soak-test.ts contains `RSS_DELTA_FAIL_THRESHOLD_MB = 200`
    - apps/desktop/scripts/soak-test.ts contains `EVENT_LOOP_P99_FAIL_THRESHOLD_MS = 50`
    - apps/desktop/scripts/soak-test.ts contains `fetchDiagnostics` function with `fetch(`${GATEWAY_URL}/internal/diagnostics``
    - apps/desktop/scripts/soak-test.ts contains `generateHtmlReport` function
    - apps/desktop/scripts/soak-test.ts contains `chart.js@4.4.0` CDN URL
    - apps/desktop/scripts/soak-test.ts contains `--duration` argument parsing logic
    - apps/desktop/scripts/soak-test.ts does NOT contain `process.memoryUsage()` calls outside of `collector.ts` (all metrics come from HTTP)
    - apps/desktop/scripts/soak-test.ts does NOT contain `HEAP_DELTA_FAIL_THRESHOLD_MB = 10` (old Phase 44 value)
    - File compiles with tsc (or node can at least parse it): `node --check apps/desktop/scripts/soak-test.ts` exits 0
  </acceptance_criteria>
  <done>
    Refactored soak-test.ts polls GET /internal/diagnostics, applies all 4 QA-01 thresholds, generates HTML report, and supports --duration flag for quick smoke tests.
  </done>
</task>
...
1. Constant check: `grep "HEAP_DELTA_FAIL_THRESHOLD_MB = 100" apps/desktop/scripts/soak-test.ts` returns match
2. Chart.js CDN reference: `grep "chart.js@4.4.0" apps/desktop/scripts/soak-test.ts` returns match
3. No local process.memoryUsage: `grep "process.memoryUsage" apps/desktop/scripts/soak-test.ts` returns NO match (metrics come from HTTP)
4. HTTP polling: `grep "GATEWAY_URL" apps/desktop/scripts/soak-test.ts` returns match
5. Duration flag: `grep -- "--duration" apps/desktop/scripts/soak-test.ts` returns match
</verification>

<success_criteria>
- soak-test.ts polls HTTP instead of measuring locally
- All 4 QA-01 thresholds applied (100MB heap, 200MB RSS, 50ms p99, ctx=1)
- HTML report generated at completion with Chart.js charts and threshold lines
- `--duration 60000` enables 1-minute smoke tests
- Script exits 0 on PASS, 1 on FAIL
</success_criteria>

<output>
After completion, create `.planning/phases/56-always-listening-soak-test/56-02-SUMMARY.md`
</output>
