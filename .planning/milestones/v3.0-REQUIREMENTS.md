# Requirements Archive: v3.0 Agentic JARVIS
...
- [x] **MCP-CLI-01**: Usuário pode configurar URL de servidor MCP estático via `.env` (HTTP transport, ex: n8n)
- [x] **MCP-CLI-02**: JARVIS usa as tools do servidor MCP configurado em conversas normais, sem configuração adicional
- [x] **MCP-CLI-03**: JARVIS descobre e registra tools disponíveis do servidor MCP ao conectar, repassando-as ao agente LLM

### Agentic Tasks

- [x] **AGENT-01**: Usuário pode solicitar tarefa multi-step por texto ou voz e JARVIS executa a sequência completa sem interrupção
- [x] **AGENT-02**: JARVIS exibe plano de execução com etapas e pede confirmação do usuário antes de iniciar
- [x] **AGENT-03**: Progresso em tempo real visível no chat e no orb a cada etapa da tarefa em execução
- [x] **AGENT-04**: Usuário pode cancelar tarefa em execução a qualquer momento sem efeitos colaterais
...
| MCP-CLI-01 | Phase 65 | Complete |
| MCP-CLI-02 | Phase 65 | Complete |
| MCP-CLI-03 | Phase 65 | Complete |
| AGENT-01 | Phase 66 | Complete |
| AGENT-02 | Phase 66 | Complete |
| AGENT-03 | Phase 66 | Complete |
| AGENT-04 | Phase 66 | Complete |
